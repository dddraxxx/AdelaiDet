...
def build_mask_branch3d(cfg, input_shape):
    return MaskBranch3D(cfg, input_shape)


class MaskBranch3D(nn.Module):
    ...
    def forward(self, features, gt_instances=None):
        for i, f in enumerate(self.in_features):
            if i == 0:
                ...
                assert target_w % w == 0
                assert target_s % s == 0
                factor_s, factor_h, factor_w = target_s, target_h // h, target_w // w
                assert factor_h == factor_w
                x_p = aligned_bilinear3d(x_p, factor_h)
                x = x + x_p
