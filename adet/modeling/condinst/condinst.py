...
def unfold_wo_center_3d(x, kernel_size, dilation):
    '''
    x: N, C, S, H, W'''
    assert x.dim() == 5
...
def get_images_color_similarity_3d(images, image_masks, kernel_size, dilation):
    assert images.dim() == 5
    assert images.size(0) == 1

    unfolded_images = unfold_wo_center_3d(
    ...
    unfolded_weights = unfold_wo_center_3d(
...
class CondInst3D(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        # modified
        # normalize images
        images_norm = [((x)) for x in original_images]
    ...
    def add_bitmasks_from_boxes_3d(self, instances, images, image_masks, im_s, im_h, im_w):
        # print(images.shape)
